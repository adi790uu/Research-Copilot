...
async def test_publish_then_subscribe_replays_log() -> None:
    ...
    await bus.publish(NodeStarted(session_id="s1", node="planner", attempt=1))
...
async def test_new_events_reach_existing_subscribers() -> None:
    ...
    await bus.publish(NodeStarted(session_id="s2", node="planner"))
...
async def test_terminal_event_sends_sentinel_and_blocks_late_subscribers() -> None:
    ...
    await bus.publish(NodeCompleted(session_id="s3", node="assembler", duration_ms=12))
...
async def test_unsubscribe_removes_queue() -> None:
    ...
    await bus.publish(NodeStarted(session_id="s5", node="planner"))
